...
class DocumentParser:
        ...
        if isinstance(document, str):
            ...
            if self.service == ServiceType.LANDING_AI:
                with fitz.open("pdf", pdf_bytes) as doc:
                    ...
                    logger.info(f"Converted PDF size: {len(pdf_bytes) / (1024*1024):.2f} MB")
                    return pdf_bytes
        else:
    ...
    def parse(self, plot: bool = False) -> Union[ParseResponse, dict]:
        ...
        if isinstance(self.document, str):
            mock_response_pkl_path = self.document.replace(".pdf", ".pkl").replace("Files", "LandingAIParsedMocks")
            if os.path.exists(mock_response_pkl_path):
                with open(mock_response_pkl_path, "rb") as file:
                    parsed_response = pickle.load(file)
                logger.info(f"Mock parsed response loaded from {mock_response_pkl_path}.")
            else:
        ...
        if plot:
            if not isinstance(self.document, str) or not self.document.lower().endswith(".pdf"):
                logger.warning("Plotting is only supported for PDF files specified by path.")
            ...
            try:
                ...
                for chunk in parsed_response.chunks:
                    ...
                    color = CHUNK_TYPE_COLORS.get(chunk_type, CHUNK_TYPE_COLORS["default"])
